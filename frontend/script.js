...
async function fetchAIWordsFromBackend(theme = "animals", count = 6) {
  try {
...
async function startGame() {
  ...
  // Fetch words from AI first
  const fetchedWords = await fetchAIWordsFromBackend('animals', words);

  // If no words are fetched from AI, use the fallback
  selectedWords = fetchedWords.length ? fetchedWords.map(w => w.toUpperCase()) : ["APPLE", "BANANA", "ORANGE"];
